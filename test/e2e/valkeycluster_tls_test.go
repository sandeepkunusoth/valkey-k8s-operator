...
var _ = Describe("ValkeyCluster mTLS with CN-based ACL mapping", Ordered, Label("ValkeyCluster", "TLS", "mTLS"), func() {
	const (
		...
		bootstrapIssuer   = "selfsigned-bootstrap-mtls"
	...
	BeforeAll(func() {
		...
		// root: a self-signed bootstrap Issuer mints a CA Certificate, and
		...
		By("creating bootstrap Issuer, root CA Certificate, CA Issuer, server Certificate, and client Certificate (CN=alice)")
		manifest := fmt.Sprintf(`
apiVersion: cert-manager.io/v1
kind: Issuer
metadata:
...
spec:
  ...
  issuerRef:
    ...
    group: cert-manager.io
`, bootstrapIssuer,
			caCertName, caCertName, bootstrapIssuer,
...
`, clusterName, serverCertSecret)
	...
	AfterAll(func() {
		...
		_ , _ = utils.Run(exec.Command("kubectl", "delete", "issuer", bootstrapIssuer, "--ignore-not-found=true"))
